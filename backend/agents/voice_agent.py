...
# ─────────────────────────────────────────────
def transcribe_audio(audio_bytes: bytes) -> tuple[str, float, str]:
    """
    ...
    import requests
    try:
        url = f"https://api.deepgram.com/v1/listen?model={settings.STT_MODEL}&detect_language=true"
        headers = {
...
1. **File civic complaints** — potholes, garbage, water supply issues, street lights, stray animals, noise, encroachment, building issues
2. **Track existing complaints** by their tracking ID (format: JS-YYYYMMDD-XXXXXX)
3. **Provide information** about which department handles what type of issue

When a citizen reports an issue, you MUST gather:
- The **issue description** (what the problem is)
- The **location** (street, area, colony, landmark)
- The citizen's **name** (or they can remain anonymous)

You can extract ALL of these from a single utterance if the citizen provides them. Do NOT ask rigid step-by-step questions if the citizen already gave the information.

Once you have enough details, use the `register_grievance` tool to file the complaint.

IMPORTANT RULES:
- Be empathetic, professional, and concise
- Keep responses SHORT (2-3 sentences max) since they will be spoken aloud
- If the citizen speaks Hindi or Telugu, respond in the SAME language
- Always confirm details before registering
...
TOOLS = [
    ...
    },
    {
        ...
        'function': {
            ...
            'description': 'Register a new grievance in the system. Call this ONLY after you have the issue description, location, and citizen name (or they chose anonymous).',
            'parameters': {
                'type': 'object',
                'properties': {
                    ...
                    'citizen_name': {'type': 'string', 'description': 'Citizen name or Anonymous'},
                    'language': {'type': 'string', 'enum': ['en', 'hi', 'te'], 'description': 'Language of the conversation'},
                },
                'required': ['description', 'location', 'citizen_name', 'language'],
            },
        },
    },
    {
        ...
        'function': {
            ...
            'description': 'Look up the status of an existing complaint by its tracking ID.',
            'parameters': {
                'type': 'object',
                'properties': {
                    'tracking_id': {'type': 'string', 'description': 'The complaint tracking ID (format: JS-YYYYMMDD-XXXXXX)'},
                },
...
# ─────────────────────────────────────────────
def _execute_tool(tool_name: str, args: dict, db) -> str:
    ...
    try:
        if tool_name == 'classify_grievance':
            ...
            result = intake_agent.run(args['description'], 'en', db)
            return json.dumps({
            ...
            g = process_new_grievance(
                ...
                ward_id=None,
                citizen_name=args.get('citizen_name', 'Anonymous'),
                citizen_phone='',
                citizen_email='',
                channel='IVR',
                db=db,
            ...
            tid = args['tracking_id'].upper().strip()
            g = db.query(Grievance).filter(Grievance.tracking_id == tid).first()
            if not g:
                return json.dumps({'found': False, 'message': f'No complaint found with ID {tid}'})
            ...
            dup = _check_duplicate(args['description'] + ' ' + args.get('location', ''), db)
            if dup:
...
def process_voice_intent(text: str, session_id: str, db, lang: str = None) -> tuple[str, str, dict]:
    ...
    if not session['messages']:
        session['messages'].append({'role': 'system', 'content': SYSTEM_PROMPT + lang_hint})
        ...
        for _ in range(3):
            ...
            for tool_call in message['tool_calls']:
                ...
                try:
                    ...
                    fn_args = {}
                print(f'[Voice Agent] Tool call: {fn_name}({fn_args})')
            ...
            if response.status_code != 200:
                print(f'[Voice Agent] LLM follow-up error {response.status_code}: {response.text[:200]}')
                return _fallback_response(text, current_lang, metadata)
                
        return final_content, agent_used, metadata
    
    except Exception as e:
        print(f'[Voice Agent] LLM exception: {e}')
        return _fallback_response(text, current_lang, metadata)
