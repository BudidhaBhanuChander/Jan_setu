...
def generate_acknowledgement(citizen_name: str, tracking_id: str, category: str,
    ...
    if isinstance(sla_deadline, str):
        try:
            ...
            sla_str = sla_dt.strftime("%d %b %Y, %I:%M %p")
        except:
            sla_str = sla_deadline
    elif hasattr(sla_deadline, 'strftime'):
        sla_str = sla_deadline.strftime("%d %b %Y, %I:%M %p")
    else:
        sla_str = "TBD"
...
def generate_status_update(tracking_id: str, new_status: str, officer_name: str = "Officer",
                            department: str = "GHMC", sla_hours: int = 24, language: str = "en",
                            citizen_name: str = "Citizen") -> dict:
    t0 = time.time()
